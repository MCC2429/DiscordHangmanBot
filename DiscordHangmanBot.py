...
You can use k.hmeasy to begin a game with and "easy" word or k.hmhard to begin a game with a "hard" word.
...
async def hm_easy(ctx):
    ...
    if not game_on:
        ...
        global hm_word
        hm_word = word_choice(wordlist)
        await ctx.send("A game of hangman has started!  Can " + str(ctx.author.mention) + " and the rest of " + str(ctx.guild) + " save the hangman?")
...
async def letter_guess(ctx):
    ...
    if not game_status:
        ...
        while True:
            ...
            try:
                re.search('k\.hml\s*([a-z])$', guess, re.I).group(1)
...
def replace_uscore(letter, hm_word, guessed_letters):
    ...
    # print(new_num_of_chars)
    # wincons(new_num_of_chars)
...
def word_choice(wordlist):
    word_list = []
    for word in wordlist:
        word_list.append(word)
    hm_word = random.choice(word_list)
    wordlist.close()
    return hm_word
